    ...
    if request.method == 'POST':
        ...
        try:
            ...
            document = Document.objects.create(
                ...
                storage_key=f'uploads/{organization.id}/{storage_key}',
            ...
            # Process with OpenAI Vision only
...
def ocr_evidence_detail_view(request, evidence_id):
    ...
    # Get associated ExtractedData and AI Summary
    extracted_data = None
    try:
        extracted_data = ExtractedData.objects.get(document=evidence.document)
...
@login_required
def ai_audit_reports_view(request):
    """
    تقارير التدقيق الذكية - AI Audit Reports
    Display AI-generated audit summaries for all documents
    """
    ...
    # Get all ExtractedData with summaries
    reports = ExtractedData.objects.filter(
        organization=organization,
        audit_summary__isnull=False
    ).exclude(audit_summary__exact='')
    ...
    # Apply status filter (from audit_summary.final_status)
    status_filter = request.GET.get('status')
    # Note: Would need to use raw SQL or Python filtering for JSON field values
    ...
    # Count reports by status (need Python-side filtering)
    approved_count = 0
    review_count = 0
    ...
    return render(request, 'documents/ai_audit_reports.html', context)


@login_required
def invoice_audit_report_view(request, extracted_data_id):
    """
    تقرير تدقيق الفاتورة الشامل - Comprehensive Invoice Audit Report
    Displays full audit report with all extracted data and findings
    """
    from documents.models import ExtractedData, OCREvidence
    ...
    except OCREvidence.DoesNotExist:
        pass
    ...
    return render(request, 'documents/invoice_audit_report.html', context)
