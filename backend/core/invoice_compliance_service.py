...
class InvoiceComplianceService:
    ...
    @staticmethod
    def _is_valid_zatca_tin(tin: str) -> bool:
        """Return True if *tin* matches ZATCA format: 15 digits, starts and ends with 3."""
        return (
            len(tin) == 15
            and tin.isdigit()
            and tin[0] == "3"
    ...
    def _check_vat_tin(self, data: dict) -> None:
        """Check 6: VAT/TIN presence and ZATCA format validity (SAR invoices)."""
        ...
        missing_count = sum(1 for t in (vendor_tin, customer_tin) if not t)
        ...
        if missing_count == 2:
            ...
            # Both TINs present — validate ZATCA format for SAR invoices
            invalid_tins = [
                t for t in (vendor_tin, customer_tin)
                if not self._is_valid_zatca_tin(t)
            ]
            if invalid_tins:
                check = ComplianceCheck(
                    check_name=check_name,
                    ...
                    severity=ComplianceCheck.SEVERITY_ERROR,
                    message=(
                        f"Invalid ZATCA TIN format (must be 15 digits, start and end with 3): "
                        f"{', '.join(invalid_tins)}"
                    ),
                )
            else:
                check = ComplianceCheck(
                    ...
                    status=ComplianceCheck.CHECK_PASS,
                    severity=ComplianceCheck.SEVERITY_INFO,
                    message="Both ZATCA TINs present and valid",
                )
        else:
            check = ComplianceCheck(
                check_name=check_name,
...
# Alias for backwards-compat and test imports
InvoiceComplianceCheckService = InvoiceComplianceService
